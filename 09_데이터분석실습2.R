# R 프로그램에서 사용자 정의 함수 제작 방법
# 함수이름 = funcion(x){
#     함수의 내용
# }
# x 는 매개변수(벡터, 값 , 데이터프레임 등이 전달 될수 있음)
# 별도의 리턴 명령은 없고, 함수의 마지막 명령 실행결과가 
# 함수이름으로 전달되어 그값이 곧 리턴 값이 됩니다

# 시:분:초 형식의 기록이 있는 데이터 프레임 에서
# 데이터 프레임 이름과, 시:분:초 형식의 데이터 필드명을 전달인수로
# 하여 초로 환산된 시간을 리턴하는 함수
...
# 2.각데이터 프레임에 각 참가년도를 저장하는 Year 열을 생성하세요
