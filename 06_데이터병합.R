...
# 행단위의 병합 - 병합하고자 하는 데이터를 제한없이 붙여 병합
rbind(aaa, bbb)
rbind(aaa, bbb,aaa, bbb,aaa, bbb)
# 행단위의 병합은 병합할 데이터들의 열갯수가 일치해야 가능합니다
...
# 행 또는 열갯수가 맞지 않으면 rbind 든 cbind 든 모두 에러입니다
aaa  = aa[1:5, 2:4]
bbb = aa[9:11, 2:4]
# cbind(aaa, bbb) 에러
aaa  = aa[1:3, 1:5]
...
# '%' 기호를 붙여서 표시(문자열함수는 다음단원에서 자세히 학습)
